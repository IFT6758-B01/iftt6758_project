...
if not current_dirpath.parts[-3:] == ('ift6758', 'advanced_models', 'decision_tree'):
    raise Exception(
        'It appears that this file is executed from the wrong location\n'
        'Expected path: (root-->)ift6758/advanced_models/decision_tree/\n'
...
wandb.init(entity="IFT6758_2024-B01" ,project="ms2-decision-tree-models")
run = wandb.run
...
y_probs = dt_clf.predict_proba(X_val)


# saving decisionTreeClassifer model to pkl file
model_basename = 'dt_classifier_complex.pkl'
model_path = pathlib.Path.cwd() / model_basename

# Save the model to a file 

joblib.dump(dt_clf, model_basename) 
