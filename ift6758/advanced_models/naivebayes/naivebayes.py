...
def roc_curve_and_auc(y_val, y_prob, log_to_run=None, commit=True):
    ...
    plt.plot(fpr, tpr, label=f"Gaussian NaiveBayes Classifier (AUC = {roc_auc:.2f})")
...
def load_dataset(return_paths: Tuple[str, str] = False):
    ...
    if not current_dirpath.parts[-3:] == ('ift6758', 'advanced_models', 'naivebayes'):
        raise Exception(
...
run = wandb.init(entity="IFT6758_2024-B01" ,project="ms2-naivebayes-models")
