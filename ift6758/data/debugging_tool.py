...
class NHLDataViewer:
    def __init__(self):
          self.nhl_data = NHLDataFetcher(base_url = "https://api-web.nhle.com/v1/gamecenter/{}/play-by-play")      
          self.select_season = ''
    ...
    def visualize_events(self,season, game_type, game_id, i):
        ...
        if(game_type == 'regular'):
            ...
            files = [f for f in os.listdir(self.nhl_data.save_dir+'/'+season) if f.startswith(file_prefix)]
        ...
        game_data = self.nhl_data.get_game_data_by_id(game_id)
    ...
    def create_interactive_viewer(self):
        ...
        def update_game_dropdown(change):
            ...
            if(game_type_dropdown.value == 'regular'):
                game_type_value = '02'
            file_prefix = f"game_{season}{game_type_value}"
            max_number = len([f for f in os.listdir(self.nhl_data.save_dir+'/'+season) if f.startswith(file_prefix)])
            if(max_number <= 0):
